...
********************************************************************
 ...
 Change History:
  ...
  1.1   Added code for 16x2 display
...
/** VECTOR REMAPPING ***********************************************/
	...
	void YourHighPriorityISRCode()
	{
			...
			TMR0L = TimeDelay & (255); //Set Delay On Timer
			TMR0H = (TimeDelay >> 8);
			mLED_3_Toggle();
			Control_BIT_Toggle();
...
{   
	...
	int MG_Flag = 0;
	char String[16];
	char Version[64] = "QFI TMTD Firmware Version 1.00                  ";
	TimeDelayPointer = &TimeDelay;
	AD_ValuePointer = &AD_Value;
	TimeDelay = 0;
    InitializeSystem();

	//Check Version (if 16x1 or 16x2 display
	// 1 = 16x2 display
	// 0 = 16x1 diaplay
    ...
    while(1)
    {
	 ...
	 if (MG_Switch == 0)
		...
		if (MG_Flag == 0)
			...
			if (DisplayVersion == 0)
				{
				x3 = sprintf(String, "   MOVIE");
				RS = 0;	
				WriteLCD(0x80);
				LCDput_str(String);
				WriteLCD(0xC0);
				x3 = sprintf(String, " GATE123        ");
				LCDput_str(String);
				}
			else
				{
				x3 = sprintf(String, "   MOVIE GATE   ");
				RS = 0;	
				WriteLCD(0x80);
				LCDput_str(String);
				}
			...
			if ( MG_High == 1)
				{
				WriteLCD(0xC0);
				x3 = sprintf(String, "      HIGH        ");
				LCDput_str(String);
				}
			else
				{
				WriteLCD(0xC0);
				x3 = sprintf(String, "      LOW       ");
				LCDput_str(String);
				}
			}
		}
		...
		if (DC_Switch == 0)  //In DC Mode Measure Current
			{
			MG_Flag = 0;
			if (DC_Mode == 0) // Just cam from pulse mode set stuff
				{
				DC_Mode = 1;
		...
		else        // In Osscilate Mode...set frequency
			{
			if (DC_Mode == 1) //Just came from DC Mode set stuff
				{
				DC_Mode = 0;
			...
			if (MG_Flag == 1) //Just came from movie gate set stuff
				{
				MG_Flag = 0;
				T0CONbits.TMR0ON = 1; //Timer On
				AD_Value = SampleFrequencyAD();
			...
			if (count == 10) // check a/d every to cycles
				{
				AD_Value = SampleFrequencyAD();
				count = 0;
				if (((AD_Value_Old - AD_Value) <= 10) && ((AD_Value_Old - AD_Value) >= -10) )
					{
					//If no change in AD go on with program
					}
				else
					{
					//iF AD change set new frequency
					frequency = SetFrequency(TimeDelayPointer, AD_ValuePointer);
					AD_Value_Old = AD_Value;
...
//	LCDput_ch('s');
		...
		ADCON1bits.PCFG0 = 0;//SET AN0, AN1, AN2, AN3, AN4, AN5, AN6 as analog
	...
	//	ADCON2bits.ACQT1 = 0;//2 tad
	//	ADCON2bits.ACQT2 = 0;
		
		ADCON2bits.ACQT0 = 1;// Aquisition time
		ADCON2bits.ACQT1 = 0;//8 tad
		ADCON2bits.ACQT2 = 0;
	...
	//	ADCON2bits.ADCS2 = 1;         
		...
		ADCON2bits.ADCS2 = 1;         
			...
			T0CONbits.T0PS2 = 1; //256 PRESCALE
			T0CONbits.T0PS1 = 1; 
			T0CONbits.T0PS0 = 1; 
