...
static const SCMD cmd[] = {
   "PIDWAIT", "On time for Athena",            setread_int,   &kAthenaOnDelay,     0,
   "CWAIT",  "On time for the Chiller",        setread_int,   &chiller_TimerMAX,   1,
   "POLL",   "Time the system is polled",      setread_int,   &system_Poll,        2,
   "FANMIN", "min time for fan",               setread_int,   &min_fan_on_time,    3, 
   "FANON",  "Force Fan On",                   setread_int,   &force_fan_on,       4,
   "Mode",   "0=USB, 1=Neslab, 2=Julabo",      setread_int,   &thermal_mode,       5,
   "FHT",     "Force High Temp",               setread_int,   &FHT,                6,
   //"FBOUNCE", "Fan Bounce Time",               setread_int,   &fanbounce_Time,     7,
   "STEMP",   "Switch temp",                   setread_float, &switch_Temp,        0,
   "SHYST",   "Switch temp hyst",              setread_float, &switch_Hyst,        1,
   "START",   "Start Temp",                    setread_float, &kDefaultAthenaTemp, 2,
   //"SET",     "Set Temp",                      set_Temp,      &dummy,              0,
   "STAGE",   "Stage Temp",                    stage_Temp,    &dummy,              0,
   "HELP",    "Help",                          help,          &dummy,              0,
};
