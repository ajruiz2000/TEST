...
void proccess_command(void){
    ...
    if (!cmd_exectuted){
        parse_Thermal(&command_to_process[0]);
    }
...
void parse_Athena_packet(void)
{
    ...
    if(0)
	{
        Add_TX_CLI_string(&GL_ERROR[0]);
		return;
	}
	// Print out the normal version string
	for(i=0; i <kMaxWrite ; i++)
	{
		data[i] = command_to_process[cmd_get - 1];
        cmd_get++;
		if(data[i] == '\r')
		{break;}
	}
	...
	if(i >= kMaxWrite)
	{
		Add_TX_CLI_string(&BC_ERROR[0]);
		return;
	}
	
	AthenaProcessExternal(data);
    //write the Athena response
    Add_TX_CLI_string(&data[0]);
