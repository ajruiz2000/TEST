...
char NLInit(void)
{
	...
	mPORTCSetBits(kNesLabsDIO); // Turn on Athena
...
char NLSetSetTemp(float value)
{
	...
	error = WriteNLArray(nlSetData.array, kSDSize);
...
float NLGetSetTemp(void)
{
	...
	//send out query
	if (WriteNLArray(nlQueryData.array, kQDSize)) //return is put into nlResData??
	{
...
// 0x08 in data2 indicates no external sensor
	if(nlStatusData.Bytes.data1 || (nlStatusData.Bytes.data2)^0x08 || nlStatusData.Bytes.data3) 
	{return 1;}
	return 0;
}
...
char WriteNLArray(char array[], int size)
{
	...
	cPointer = nlResData.array;
//write data with timeout running
//read data with timeout running
//if timeout or return bad (at a minimum test checksum against expected and check for xOF in bath error byte
//if command, array[3] is x09, read 11 bytes and put into nlStatusData
//else read 9 bytes and put into nlResData
//if return[3] is x0F, only read 8 bytes and return error
...
//for testing - return of NLGetTemp
//	nlResData.Bytes.ca = 0xCA;
//	nlResData.Bytes.ID1 = 0x00;
//	nlResData.Bytes.ID2 = 0x01;
//	nlResData.Bytes.command = 0x20;
//	nlResData.Bytes.bytes = 0x03;
//	nlResData.Bytes.qb = 0x11; 			//qualifer byte - indicates data precision
//	nlResData.Bytes.data1 = 0x01;
//	nlResData.Bytes.data2 = 0x2C;
//	nlResData.Bytes.cs = 0x9D;	

//for testing - return of NLGetStatus
//	nlStatusData.Bytes.ca = 0xCA;
//	nlStatusData.Bytes.ID1 = 0x00;
//	nlStatusData.Bytes.ID2 = 0x01;
//	nlStatusData.Bytes.command = 0x09;
//	nlStatusData.Bytes.bytes = 0x05;
//	nlStatusData.Bytes.data1 = 0x00;
//	nlStatusData.Bytes.data2 = 0x02;
//	nlStatusData.Bytes.data3 = 0x00;
//	nlStatusData.Bytes.data4 = 0x00;
//	nlStatusData.Bytes.data5 = 0x00;
//	nlStatusData.Bytes.cs = 0xEE;	

	if (array[kCommandByte] == 0x09)//nlResData.array[3] indicates potential change in data length
	{
		maxBytes = kStDSize;
		cPointer = nlStatusData.array;
	}
	...
	for(i=0; i<maxBytes; i++)
	{
		...
		*(cPointer + i) = getcUART1();
		if((i == kCommandByte) && (*(cPointer + kCommandByte) == 0x0F))
	...
	cs = GetCheckSum(cPointer, maxBytes);
	if (*(cPointer + maxBytes - 1) != cs)
	{return 1;}
