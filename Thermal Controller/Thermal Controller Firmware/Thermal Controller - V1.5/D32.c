...
void EnableSerialPort(void)
	{
	OpenUART1(UART_EN|UART_NO_PAR_8BIT|UART_1STOPBIT, UART_RX_ENABLE|UART_TX_ENABLE,520); //518
	}

int CheckSerialPort(void)
	{
	static int test = 0;
	static int i = 0;
	int j;
...
//if (U1STAbits.OERR == 1)
	...
	if (test == 1)
		...
		for (i=0;i<=24; i++)
			...
			//	U1STAbits.OERR = 0;                    //Clears the OERR bit so UART clears buffer and 
				SerialCommand[i] = getcUART1();
			//	test = DataRdyUART1();
			//	i++;
				if (SerialCommand[i] == '\r')
					{
					break;
					}
				//test = DataRdyUART1();
				while (!UARTReceivedDataIsAvailable(UART1))
					{
					//sit and spin tillnext character ready
					}	
				}
		}	
		else 
		...
		return 0;
			...
			case '\r':
				...
				switch (SerialCommand[0])
					...
					for(j=0; j<64; j++)
						{
						if (SerialCommand[1] == 'V' || SerialCommand[1] == 'v')
							{
							putcUART1((const char *)st_version[j]);
							if((const char *)st_version[j] == '\r')
								{break;}
							}
						}
					break;
				...
				//	for(j=0; j<64; j++)
					//	{
					//	SerialCommand[j] = SerialCommand[j+1];
				//		if (SerialCommand[j] == '\r')
					//		{break;}
					//	}
					AthenaProcessExternal(SerialCommand);
					for(j=0; j<16; j++)
						{
					//	 if (U1STAbits.OERR == 1)
						//	{
